...
class UserBoardController {
  ...
  // ----------------------------------------------------
  // Assign a file to the user board
  static async assignFile(req, res) {
    try {
      const { userBoardId } = req.params;
      const fileData = req.body;
      const userBoard = await UserBoardModel.assignFile(userBoardId, fileData);
      res.status(200).json({
      ...
      res.status(500).json({
        error: error.message,
        info: "Failed to assign file to user board.",
      });
    }
  }

  // ----------------------------------------------------
  // Remove an assigned file by fileID from the user board
  static async removeAssignedFile(req, res) {
    try {
      const { userBoardId, fileID } = req.params;
      const userBoard = await UserBoardModel.removeAssignedFile(
        userBoardId,
        fileID
      );
      res.status(200).json({
        message: `File with ID '${fileID}' removed successfully`,
        data: userBoard,
      });
    } catch (error) {
      res.status(500).json({
        error: error.message,
        info: "Failed to remove assigned file.",
      });
    }
  }
